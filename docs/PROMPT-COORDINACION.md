...
4. **No modifiques archivos compartidos** (`package.json` en sus dependencias raíz, `vite.config.ts`, layout/navegación raíz, `.env.example`, `docker-compose.yml`) sin decírselo antes al usuario — otros módulos dependen de que no cambien por sorpresa.
